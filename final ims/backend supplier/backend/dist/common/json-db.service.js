    ...
    readFromDisk() {
        try {
            ...
            return {
                ...
                stores: Array.isArray(parsed.stores) && parsed.stores.length
                    ? parsed.stores
                    : structuredClone(seed_data_1.INITIAL_DB.stores),
                customers: Array.isArray(parsed.customers) && parsed.customers.length
                    ? parsed.customers
                    : structuredClone(seed_data_1.INITIAL_DB.customers),
                billers: Array.isArray(parsed.billers) && parsed.billers.length
                    ? parsed.billers
