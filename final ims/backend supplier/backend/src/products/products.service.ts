...
export class ProductsService {
  ...
  findOne(id: string): ProductRecord {
    const product = this.getProducts().find((entry) => entry.id === id);
    if (!product) {
  ...
  findBySku(sku: string): ProductRecord {
  ...
  create(createProductDto: CreateProductDto): ProductRecord {
    ...
    const sku = this.normalizeSku(
      createProductDto.sku || this.generateNextSku(products),
    );
    this.ensureUniqueSku(products, sku);
  ...
  update(id: string, updateProductDto: UpdateProductDto): ProductRecord {
    ...
    this.ensureUniqueSku(products, nextSku, existing.id);
  ...
  getFeedback(sku: string) {
    return this.findBySku(sku).feedback.slice().sort((a, b) => {
      return new Date(b.date).getTime() - new Date(a.date).getTime();
    });
  }

  getRatingSummary(sku: string) {
  ...
  applyInventoryAdjustment(
    ...
    const products = this.getProducts();
    const index = products.findIndex((entry) => entry.sku === sku);
    if (index === -1) {
      throw new NotFoundException('Product not found');
    }
  ...
  private resolveStoreInventory(
    ...
    if (Array.isArray(nextEntries) && nextEntries.length) {
      return this.normalizeStoreInventory(nextEntries, totalQty);
    }
    ...
    return this.normalizeStoreInventory(undefined, totalQty);
  }
  ...
  private normalizeStoreInventory(
    ...
    const defaultStoreIds = ['s1', 's3', 's7'];
    const source =
      Array.isArray(entries) && entries.length
        ? entries
        : defaultStoreIds.map((storeId, index) => ({
            storeId,
            qty:
              index === defaultStoreIds.length - 1
                ? totalQty -
                  Math.floor(totalQty * 0.35) -
                  Math.floor(totalQty * 0.28)
                : Math.floor(totalQty * [0.35, 0.28][index] || 0),
          }));
  ...
  private generateNextSku(products: ProductRecord[]) {
    const nextValue =
