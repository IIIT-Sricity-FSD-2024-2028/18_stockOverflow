(function () {
  ...
  async function getProducts(force) {
    ...
    cache.products = await window.IMS_HTTP.request('/products');
    return clone(cache.products);
  }
  ...
  async function getProductBySku(sku, force) {
    ...
    return window.IMS_HTTP.request('/products/sku/' + encodeURIComponent(sku));
  }
  ...
  async function getStores(force) {
    ...
    cache.stores = await window.IMS_HTTP.request('/stores');
    return clone(cache.stores);
  }
  ...
  async function getCustomers(force) {
    ...
    cache.customers = await window.IMS_HTTP.request('/customers');
    return clone(cache.customers);
  }

  async function getReservations(storeId) {
    var suffix = storeId ? '?storeId=' + encodeURIComponent(storeId) : '';
    return window.IMS_HTTP.request('/reservations' + suffix);
  }

  async function getReservationRequests(status) {
    var suffix = status ? '?status=' + encodeURIComponent(status) : '';
    return window.IMS_HTTP.request('/reservations/requests' + suffix);
  }
  ...
  async function getTransactions() {
    return window.IMS_HTTP.request(withCustomerQuery('/transactions'));
  }

  async function getLatestTransaction() {
    return window.IMS_HTTP.request(withCustomerQuery('/transactions/latest'));
  }
  ...
  async function getTransactionByOrderId(orderId) {
    ...
    return window.IMS_HTTP.request(
      withCustomerQuery('/transactions/' + encodeURIComponent(orderId)),
    );
  }
  ...
  async function getPurchasedProducts() {
    return window.IMS_HTTP.request(
      withCustomerQuery('/transactions/purchased-products'),
    );
  }
  ...
  async function getReturns() {
    return window.IMS_HTTP.request(withCustomerQuery('/returns'));
  }
