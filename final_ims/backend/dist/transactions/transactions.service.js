...
let TransactionsService = class TransactionsService {
    ...
    findAll(retailerId, storeId, customerLookup) {
        return this.db
            ...
            .filter((transaction) => this.matchesScope(transaction, retailerId, storeId, customerLookup))
            .sort((a, b) => {
    ...
    findOne(orderId, retailerId, storeId, customerLookup) {
        const transaction = this.findAll(retailerId, storeId, customerLookup).find((entry) => entry.orderId === orderId);
        if (!transaction) {
    ...
    create(createTransactionDto) {
        ...
        const normalized = this.buildTransactionRecord(this.enrichCustomerFromReservation(createTransactionDto), orderId);
        this.ensureInventoryAvailability(normalized.items, normalized.storeId, normalized.retailerId);
        ...
        this.customersService.registerPurchase(normalized.customer, normalized.finalTotal, normalized.retailerId, normalized.storeId, normalized.customerEmail, normalized.customerId);
        return normalized;
    }
    ...
    buildTransactionRecord(payload, orderId, timestamp = new Date().toISOString()) {
        ...
        return {
            ...
            customerEmail: this.normalizeText(payload.customerEmail),
            customerId: this.normalizeText(payload.customerId),
            store,
            storeId,
    ...
    enrichCustomerFromReservation(payload) {
        ...
        if (currentCustomer && currentCustomer !== 'walk-in customer') {
            return payload;
        }
        ...
        (Array.isArray(payload.items) ? payload.items : []).forEach((item) => {
            ...
            item.requestIds.forEach((requestId) => {
                const normalized = this.normalizeText(requestId);
                if (normalized) {
        ...
        const request = this.db.getCollection('reservationRequests').find((entry) => {
            return (requestIds.has(entry.requestId) &&
                (entry.customer || entry.customerEmail || entry.customerId));
        });
        if (!request) {
            return payload;
        }
        return {
            ...payload,
            customer: this.normalizeText(request.customer, this.normalizeText(payload.customer, 'Walk-in Customer')),
            customerEmail: this.normalizeText(request.customerEmail, payload.customerEmail),
            customerId: this.normalizeText(request.customerId, payload.customerId),
        };
    }
    normalizeItem(item) {
    ...
    ensureInventoryAvailability(items, storeId, retailerId) {
        const products = this.db.getCollection('products');
