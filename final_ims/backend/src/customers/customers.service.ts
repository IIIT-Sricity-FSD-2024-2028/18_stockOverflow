...
export class CustomersService {
  ...
  findAll(retailerId?: string, storeId?: string) {
    return this.getCustomers()
      .map((customer) => this.normalizeCustomer(customer))
      .filter((customer) => this.matchesScope(customer, retailerId, storeId));
  }

  findOne(id: string, retailerId?: string, storeId?: string) {
    const customer = this.findAll(retailerId, storeId).find(
  ...
  create(createCustomerDto: CreateCustomerDto) {
    ...
    if (
      ...
      customers.some(
        (entry) =>
          this.matchesScope(
            this.normalizeCustomer(entry),
            createCustomerDto.retailerId,
  ...
  registerPurchase(
    ...
    const index = customers.findIndex((entry) => {
      ...
      return (
        this.matchesScope(normalizedEntry, retailerId, storeId) &&
        (
          (customerId &&
      ...
      const created = this.buildCustomerRecord(
        {
          ...
          name: normalizedName,
          email:
            this.normalizeText(customerEmail) ||
        ...
        { id: this.normalizeText(customerId, `cust-${randomUUID()}`) },
      );
