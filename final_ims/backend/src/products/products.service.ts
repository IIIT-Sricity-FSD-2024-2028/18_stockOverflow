...
export class ProductsService {
  ...
  findOne(id: string, retailerId?: string, storeId?: string): ProductRecord {
    const product = this.filterProducts(
      this.getProducts(),
      retailerId,
      storeId,
  ...
  getFeedback(sku: string, retailerId?: string) {
  ...
  addFeedback(
    sku: string,
    createProductFeedbackDto: CreateProductFeedbackDto,
    retailerId?: string,
  ): ProductFeedback {
    const products = this.getProducts();
    const index = products.findIndex((entry) => {
      return (
        entry.sku.toLowerCase() === String(sku || '').toLowerCase() &&
    ...
    const feedback: ProductFeedback = {
      ...
      productName: this.normalizeText(
        createProductFeedbackDto.productName,
        product.name,
      ),
      customer: this.normalizeText(
        createProductFeedbackDto.customer,
        'Recent Buyer',
      ),
      type: this.normalizeText(createProductFeedbackDto.type, 'General'),
      rating: Math.max(
        1,
        Math.min(5, Math.trunc(createProductFeedbackDto.rating || 0)),
      ),
      comment: this.normalizeText(
        createProductFeedbackDto.comment,
        'No comment provided.',
      ),
  ...
  rebuildInventoryFromTransactions(transactions: TransactionRecord[]) {
    ...
    transactions.forEach((transaction) => {
      ...
      transaction.items.forEach((item) => {
        ...
        const targetStoreId =
          transaction.storeId || this.resolveStoreId(transaction.store);
        const inventoryEntry =
          product.storeInventory.find((entry) => entry.storeId === targetStoreId) ||
  ...
  applyInventoryAdjustment(
    ...
    const nextStoreInventory = this.adjustStoreInventory(
      product.storeInventory,
      qtyDelta,
      preferredStoreId,
    );
    const nextInitialStoreInventory = this.adjustStoreInventory(
      product.initialStoreInventory,
      qtyDelta,
      preferredStoreId,
    );
  ...
  private saveProducts(products: ProductRecord[]) {
    const normalized = products.map((entry, index) =>
      this.normalizeStoredProduct(entry, index),
    );
  ...
  private filterProducts(
    ...
    return products
      .filter((product) =>
        this.matchesRetailerScope(product.retailerId, retailerId),
      )
      .filter((product) => {
        ...
        return (
          product.storeInventory.some(
            (entry) => this.normalizeText(entry.storeId) === normalizedStoreId,
          ) ||
          product.initialStoreInventory.some(
            (entry) => this.normalizeText(entry.storeId) === normalizedStoreId,
          )
        );
      })
  ...
  private normalizeStoredProduct(entry: Partial<ProductRecord>, index: number) {
    const now = new Date().toISOString();
    ...
    const subcategory = this.normalizeText(
      entry.subcategory || entry.subCategory,
      category,
    );
    const priceUSD = this.toMoney(
      this.pickNumber(entry.priceUSD, entry.price, 0),
    );
    ...
    const galleryImages = this.normalizeImageList(
      Array.isArray(entry.galleryImages) ? entry.galleryImages : entry.images,
    );
    ...
    const storeInventory = this.normalizeStoreInventory(
      entry.storeInventory,
      qty,
      index,
    );
    const initialStoreInventory = this.normalizeStoreInventory(
      entry.initialStoreInventory && entry.initialStoreInventory.length
    ...
    const feedback = Array.isArray(entry.feedback)
      ? entry.feedback
          .filter((item): item is ProductFeedback => Boolean(item))
    ...
    const normalized: ProductRecord = {
      ...
      finalPrice: this.toMoney(
        this.pickNumber(entry.finalPrice, priceUSD, priceUSD),
      ),
      ...
      creatorImg: this.normalizeText(
        entry.creatorImg,
        this.buildAvatarSvg(this.normalizeText(entry.creator, 'Retail Manager')),
      ),
  ...
  private buildProductRecord(
    ...
    options: { id: string; sku: string; existing?: ProductRecord },
  ): ProductRecord {
    const existing = options.existing;
    const now = new Date().toISOString();
    const priceUSD = this.toMoney(
      this.pickNumber(payload.priceUSD, payload.price, existing?.priceUSD, existing?.price, 0),
    );
    const qty = this.toSafeInteger(payload.qty ?? existing?.qty ?? 0);
    const min = this.toSafeInteger(payload.min ?? existing?.min ?? 0);
    const max = Math.max(
      this.toSafeInteger(payload.max ?? existing?.max ?? 0),
      qty,
      min,
      1,
    );
    const nextStoreInventory = this.resolveStoreInventory(
      payload.storeInventory,
      existing?.storeInventory,
      qty,
      this.normalizeText(
        (payload as CreateProductDto & { storeId?: string }).storeId,
        existing?.storeInventory?.[0]?.storeId,
        existing?.initialStoreInventory?.[0]?.storeId,
        'default-store',
      ),
    );
    const previousStoreInventory = existing?.storeInventory || [];
    const initialStoreInventory = existing
      ? this.resolveInitialStoreInventory(
          existing.initialStoreInventory,
          previousStoreInventory,
          nextStoreInventory,
        )
    ...
    const nextInitialQty = existing
      ? Math.max(
          0,
          this.toSafeInteger(existing.initialQty) + (explicitQty ? qty - existing.qty : 0),
        )
      : qty;
    const baseImageCandidates = this.normalizeImageList(
      payload.galleryImages || payload.images || existing?.galleryImages || existing?.images,
    );
    const imageSet = this.buildImageSet(
      this.normalizeText(payload.productImg, existing?.productImg),
      baseImageCandidates,
      {
    ...
    const product: ProductRecord = {
      ...
      name: this.normalizeText(payload.name, existing?.name || 'Product'),
      category: this.normalizeText(payload.category, existing?.category || 'General'),
      subcategory: this.normalizeText(
        payload.subcategory || payload.subCategory,
        existing?.subcategory || existing?.subCategory || existing?.category || 'General',
      ),
      subCategory: this.normalizeText(
        payload.subcategory || payload.subCategory,
        existing?.subCategory || existing?.subcategory || existing?.category || 'General',
      ...
      discountType: this.normalizeText(
        payload.discountType,
        existing?.discountType || 'none',
      ),
      discountValue: this.toOptionalMoney(
        payload.discountValue ?? existing?.discountValue,
      ),
      ...
      creatorImg: this.normalizeText(
        payload.creatorImg,
        existing?.creatorImg || this.buildAvatarSvg(this.normalizeText(payload.creator, existing?.creator || 'Retail Manager')),
      ),
      productImg: imageSet[0],
      galleryImages: imageSet.slice(0, 3),
      images: imageSet,
      emoji: this.normalizeText(
        payload.emoji,
        existing?.emoji || String.fromCodePoint(0x1f4e6),
      ),
      soldThisMonth: this.toSafeInteger(
        payload.soldThisMonth ?? existing?.soldThisMonth ?? 0,
      ),
      ...
      descriptionHtml: this.normalizeText(
        payload.descriptionHtml,
        existing?.descriptionHtml,
      ),
      supplier: this.normalizeText(
        payload.supplier,
        existing?.supplier || existing?.brand || 'StockOverflow Supply',
      ),
      ...
      reorderPoint: this.toSafeInteger(
        payload.reorderPoint ?? existing?.reorderPoint ?? min,
      ),
      reorderQty: this.toSafeInteger(
        payload.reorderQty ?? existing?.reorderQty ?? Math.max(1, min),
      ),
      hasVariants:
        payload.hasVariants ??
        Boolean(
      ...
      shippingClass: this.normalizeText(
        payload.shippingClass,
        existing?.shippingClass || 'Standard',
      ),
  ...
  private resolveStoreInventory(
    nextEntries: Array<{ storeId: string; qty: number }> | undefined,
    existingEntries: StoreInventoryEntry[] | undefined,
    totalQty: number,
        ...
        return existingEntries.map((entry, index) => {
          const qty =
            index === existingEntries.length - 1
              ...
              : Math.max(
                  0,
                  Math.floor((totalQty * Math.max(0, entry.qty || 0)) / previousTotal),
                );
          running += qty;
          return {
  ...
  private resolveInitialStoreInventory(
    initialEntries: StoreInventoryEntry[] | undefined,
    currentEntries: StoreInventoryEntry[],
    nextEntries: StoreInventoryEntry[],
  ) {
    const base =
  ...
  private normalizeStoreInventory(
    entries: Array<{ storeId: string; qty: number }> | undefined,
    totalQty: number,
    seedIndex = 0,
    defaultStoreId = 'default-store',
  ) {
    ...
    const normalized = source.map((entry, index) => ({
      storeId: this.normalizeText(
        entry.storeId,
        defaultStoreIds[index] || `s${seedIndex + index + 1}`,
      ),
    ...
    if (normalized.length && currentTotal !== totalQty) {
      normalized[normalized.length - 1] = {
        ...
        qty: Math.max(
          0,
          normalized[normalized.length - 1].qty + (totalQty - currentTotal),
        ),
      };
    }
    return normalized;
  ...
  private normalizeImageList(values?: string[]) {
  ...
  private buildProductPlaceholder(
    fallback: { name: string; category: string; sku: string; emoji: string },
    variantIndex: number,
  ) {
  ...
  private buildAvatarSvg(name: string) {
  ...
  private normalizeRatingBreakdown(
    breakdown?: Partial<RatingBreakdown>,
  ): RatingBreakdown {
    return {
  ...
  private applyRatings(product: ProductRecord) {
    ...
    if (Array.isArray(product.feedback) && product.feedback.length) {
      ...
      product.feedback.forEach((entry) => {
        ...
        breakdown[rating as keyof RatingBreakdown] += 1;
      });
    }
  ...
  private resolveFinalPrice(
    ...
    const discountType = this.normalizeText(
      payload.discountType,
      existing?.discountType || 'none',
    ).toLowerCase();
    const discountValue = this.toMoney(
      this.pickNumber(payload.discountValue, existing?.discountValue, 0),
    );
  ...
  private ensureUniqueSku(
    ...
    const exists = products.some((entry) => {
      return (
        entry.sku.toLowerCase() === sku.toLowerCase() &&
        this.matchesRetailerScope(entry.retailerId, retailerId) &&
        entry.id !== ignoreId
      );
    });
  ...
  private generateNextSku(products: ProductRecord[], retailerId?: string) {
    const nextValue =
      products
        .filter((product) =>
          this.matchesRetailerScope(product.retailerId, retailerId),
  ...
  private projectProductForStore(product: ProductRecord, storeId?: string) {
    const normalizedStoreId = this.normalizeText(storeId);
    if (!normalizedStoreId) {
      return product;
    }
    ...
    const currentStore =
      product.storeInventory.find((entry) => entry.storeId === normalizedStoreId) ||
      null;
    const initialStore =
      product.initialStoreInventory.find(
        (entry) => entry.storeId === normalizedStoreId,
      ) || null;
  ...
  private matchesRetailerScope(
    candidateRetailerId: string | undefined,
    retailerId?: string,
  ) {
  ...
  private isSameRetailerScope(
    leftRetailerId: string | undefined,
    rightRetailerId: string | undefined,
  ) {
    return (
      this.normalizeText(leftRetailerId) === this.normalizeText(rightRetailerId)
    );
  }
  ...
  private toOptionalMoney(value: number | undefined) {
