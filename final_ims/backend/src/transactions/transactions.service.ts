...
export class TransactionsService {
  ...
  findAll(retailerId?: string, storeId?: string, customerLookup?: string) {
    return this.db
      ...
      .filter((transaction) =>
        this.matchesScope(transaction, retailerId, storeId, customerLookup),
      )
      .sort((a, b) => {
  ...
  create(createTransactionDto: CreateTransactionDto) {
    ...
    const normalized = this.buildTransactionRecord(
      this.enrichCustomerFromReservation(createTransactionDto),
      orderId,
    );

    this.ensureInventoryAvailability(
      normalized.items,
      normalized.storeId,
  ...
  private enrichCustomerFromReservation<T extends CreateTransactionDto | UpdateTransactionDto>(
    ...
    return {
      ...
      customer: this.normalizeText(
        request.customer,
        this.normalizeText(payload.customer, 'Walk-in Customer'),
      ),
