...
export class UsersService {
  ...
  private hydrateBillerProfile(user: User) {
  ...
  private findBillerProfile(user: User) {
    const billers = [
      ...this.readRecordsFromFile<BillerProfileRecord>(this.billersFile),
      ...this.readCollectionFromDbFile<BillerProfileRecord>('billers'),
    ];
    const lookupId = this.normalizeText(user.profileId);
    const lookupEmail = this.normalizeEmail(user.email);

    return (
      billers.find((biller) => String(biller.id) === lookupId) ||
  ...
  private readCollectionFromDbFile<T>(collectionKey: string) {
    try {
      if (!existsSync(this.dbFile)) {
        return [] as T[];
      }
      const raw = readFileSync(this.dbFile, 'utf-8').trim();
      if (!raw) {
        return [] as T[];
      }
      const parsed = JSON.parse(raw) as Record<string, unknown>;
      const collection = parsed[collectionKey];
      return Array.isArray(collection) ? (collection as T[]) : ([] as T[]);
    } catch {
      return [] as T[];
    }
  }
