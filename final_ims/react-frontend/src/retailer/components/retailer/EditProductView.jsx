...
export default function EditProductView({ product: initialProduct, onNavigate }) {
  const { user } = useAuth();
  ...
  // Helper to populate form fields from product
  const populateFormWithProduct = (p) => {
    ...
    const mainImg = p.productImg || p.image || '';
    const allImgs = Array.from(new Set([mainImg, ...rawImages])).filter(Boolean);
    const mappedImages = allImgs.map((src, i) => ({
      src,
      name: `Image ${i + 1}`,
    }));
    ...
    setFormData({
      ...
      min: p.min != null ? p.min : 10,
      max: p.max != null ? p.max : 500,
      warehouse: p.warehouse || '',
      bin: p.bin || '',
      reorderPoint: p.reorderPoint != null ? String(p.reorderPoint) : '',
      reorderQty: p.reorderQty != null ? String(p.reorderQty) : '',
      restockDate: p.restockedAt ? p.restockedAt.split('T')[0] : new Date().toISOString().split('T')[0],
    ...
    async function loadData() {
      try {
        ...
        if (Array.isArray(prodData)) {
          ...
          // Extract distinct categories from DB + defaults
          ...
          setCategories(Array.from(new Set([...dbCats, ...defaultCats])));

          // Extract distinct brands from DB + defaults
          ...
          setBrands(Array.from(new Set([...dbBrands, ...defaultBrands])));

          // If no initial product passed, use the first product in the database
          if (!initialProduct && prodData.length > 0) {
            populateFormWithProduct(prodData[0]);
  ...
  // Update Product on Real Backend
  const handleUpdateProduct = async () => {
    if (!currentProduct?.id) {
      triggerToast('No product selected to update.', true);
      return;
    }
    ...
    try {
      ...
      await request(`/products/${currentProduct.id}`, {
        method: 'PUT',
        body: JSON.stringify(payload),
      });

      triggerToast('Product updated successfully! Redirecting…');
      setTimeout(() => {
        onNavigate('products');
      }, 1200);
    } catch (err) {
      console.error('Error updating product:', err);
      triggerToast(err.message || 'Error updating product', true);
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="page-wrap" style={{ padding: '24px 28px 48px' }}>
      ...
      <div className="page-head">
        <div className="page-head-left">
          ...
          <div className="page-subtitle">Products / Edit Product</div>
        </div>
      </div>
            ...
            <div className="card">
              ...
              <div className="card-body dense">
                ...
                <div
                  ...
                  onClick={() => document.getElementById('imgEditUploadInput')?.click()}
                >
                  <input
                    type="file"
                    id="imgEditUploadInput"
                    accept="image/*"
                    multiple
            ...
            <div className="card">
              ...
              <div className="card-body">
                ...
                <div className="field-row col-3">
                  <div className="field">
                    <label className="field-label">
                      Current Stock <span className="req">*</span>
                    </label>
                    <input
